const PRODUCT_NAME = "Eggnog Crème";
...
function addNotifyButton() {
    if (!document.getElementById("notifyTriggerBtn")) {
        ...
        btn.setAttribute("data-bs-target", "#notifyModal"); // <- fixed
...
qtyInput.addEventListener("change", () => {
    ...
    if (newQty > currentStock) qtyInput.value = currentStock;
    else if (newQty < 1 || isNaN(newQty)) qtyInput.value = 1;
});

// --- Inline Notice ---
function showInlineNotice(message) {
    let notice = document.getElementById("eggnogInlineNotice");
    if (!notice) {
        notice = document.createElement("span");
        notice.id = "eggnogInlineNotice";
