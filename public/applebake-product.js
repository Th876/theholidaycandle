// === CHRISTMAS APPLE BAKE PRODUCT SCRIPT ===

const PRODUCT_NAME = "Christmas Apple Bake";
...
function addNotifyButton() {
    const existing = document.getElementById("notifyTriggerBtn");
    if (!existing) {
...
increaseQtyBtn.addEventListener("click", () => {
    ...
    // else showToast("Looks like you’ve reached the limit for this item.");
...
// --- 🟡 Inline Notice Beside Quantity Buttons (Horizontal) ---
function showInlineNotice(message) {
    let notice = document.getElementById("inlineNotice");
    if (!notice) {
        notice = document.createElement("span");
        notice.id = "inlineNotice";
        notice.style.backgroundColor = "#f0d169"; // warm yellow-orange, visible on light bg
...
function validateEmailNotify(email) {
    ...
    // Stronger pattern to reject fake TLDs like ".colm" or missing parts
    const re = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[A-Za-z]{2,}$/;

    // Valid domain suffixes (can extend if needed)
    const validTLDs = [
        "com", "org", "net", "edu", "gov", "info", "io", "co", "us", "ca", "uk", "biz", "shop", "store", "xyz", "email"
    ];
